...
class DaftExprtLoss(nn.Module):
    ...
    def forward(self, outputs, targets, iteration):
        ...
        if len(targets) == 2:
            # we are doing PABC
            gt_targets, ref_targets = targets
            gt_duration_targets, gt_energy_targets, gt_pitch_targets, gt_mel_spec_targets, gt_speaker_ids = gt_targets
            ref_duration_targets, ref_energy_targets, ref_pitch_targets, ref_mel_spec_targets, ref_speaker_ids = ref_targets

            duration_targets = gt_duration_targets
            energy_targets = gt_energy_targets
            pitch_targets = gt_pitch_targets
            mel_spec_targets = gt_mel_spec_targets
            speaker_ids = ref_speaker_ids
